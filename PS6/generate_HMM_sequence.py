...
def generate_HMM_sequence(hmm_file, seq_length):
    ...
    # make lists storing state sequence and observed sequence
    state_seq = []
    observation_seq = []
    # add first state based on initial probability
    state_seq.extend(random.choices(states, initial_probs))
    # add first observation nucleotide based on state
    observation_seq.extend(random.choices(input_alphabet, emission[state_seq[0]]))
    # continue generating the path and observation until the sequence is length seq_length
    while len(observation_seq) <= seq_length:
        # update state based on most recent state
        state_seq.extend(random.choices(states, transitions[state_seq[-1]]))
        # update observed sequence based on state that was just updated
        observation_seq.extend(random.choices(input_alphabet, emission[state_seq[-1]]))

    return (state_seq, observation_seq)  # a tuple containing the state and observation sequences
